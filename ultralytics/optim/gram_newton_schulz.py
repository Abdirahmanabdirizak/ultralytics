...
class GramNewtonSchulz:
    ...
    def __init__(
        ...
        self.gram_newton_schulz_reset_iterations = (
            gram_newton_schulz_reset_iterations if gram_newton_schulz_reset_iterations is not None else [2]
        )
        ...
        if X.size(-2) != X.size(-1):
            ...
            for i, (a, b, c) in enumerate(self.ns_coefficients):
                ...
                if not self.ns_use_kernels or X.size(-2) <= SYMMETRIC_KERNEL_TILE_SIZE:
                    ...
                    Q = (
                        torch.baddbmm(Q, Q, Z, beta=a)
                        if i != 0 and i not in self.gram_newton_schulz_reset_iterations
                        else Z + a * I
                    )
                    ...
                    Q = (
                        self.gemm_symmetric(Q, Z, C=Q, beta=a)
                        if i != 0 and i not in self.gram_newton_schulz_reset_iterations
                        else Z + a * I
                    )
                    if i < len(self.ns_coefficients) - 1 and i + 1 not in self.gram_newton_schulz_reset_iterations:
